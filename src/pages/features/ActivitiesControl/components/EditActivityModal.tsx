...
export default function EditActivityModal({ activity, onClose, onSave }: EditActivityModalProps) {
  const [formState, setFormState] = useState({
    ...
    tipo: activity.tipo,
    funcionario: activity.funcionario,
    talhao: activity.talhao || '',
    cultura: activity.cultura || '',
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    onSave({
      ...
      dataConclusao:
        formState.status === 'Concluída' && !activity.dataConclusao
    ...
    <div className="modal-backdrop movement-modal-backdrop" onClick={onClose}>
      ...
      >
        <header className="modal-header">
          <div>
            <h3 className="modal-title">Editar Atividade</h3>
            <p className="modal-subtitle">Atualize as informações da atividade</p>
          </div>
